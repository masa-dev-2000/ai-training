...
### 中級編 自動化設定ガイド
- [x] index.html（全41手法の一覧）→ `materials/intermediate/index.html`
  - 検索ボックス（キーワードフィルター）+ 難易度タブフィルター実装済み
  - カード説明文・stats・カテゴリカウント付き
- [x] マーケティング・広告（7手法）→ `materials/intermediate/01-marketing/`
- [x] 営業（5手法）→ `materials/intermediate/02-sales/`
- [x] 法務（3手法）→ `materials/intermediate/03-legal/`
- [x] 経理・バックオフィス（6手法）→ `materials/intermediate/04-accounting/`
- [x] カスタマーサポート（3手法）→ `materials/intermediate/05-support/`
- [x] 総務・人事（5手法）→ `materials/intermediate/06-hr/`
- [x] 開発・エンジニアリング（5手法）→ `materials/intermediate/07-engineering/`
- [x] コンテンツ制作（4手法）→ `materials/intermediate/08-content/`
- [x] 経営・全社横断（3手法）→ `materials/intermediate/09-management/`
- 充実化済みページ（完成イメージ・ケース別タブ・チェックリスト・関連ガイド付き）：
  - `01-marketing/03-sns-batch.html`（用途別4タブ、出力例付き）
  - `04-accounting/01-invoice-draft.html`（請求書プレビュー、4ケース対応）
  - `05-support/01-email-reply.html`（4シナリオタブ、送信前チェックリスト）
...
| # | タスク | 優先度 |
|---|--------|--------|
| 0 | **第1回を団体講義向けに改訂**（業務テーマ選定の運用組み込みも含む） | 高 |
| 0b | 業種別ペルソナ設定書 → `design/personas.md` | ✅ 完了 |
| 0c | **第4回を団体講義向けに改訂** → `design/session-04-org-design.md` ／ `materials/session-04/presentation-org.html` | ✅ 完了（必修・推奨・任意11件のパッチ適用済） |
| 1 | **第2回の設計書＆講義資料**（団体版で作成） | 高 |
| 2 | **presentation.htmlのJSON読み込み対応** | 高 |
| 3 | 第3回・第5〜8回の設計書＆講義資料（団体版で作成） | 中 |
| 4 | 計画届テンプレート | 中 |
| 5 | 出席管理テンプレート | 中 |
| 6 | 補講運用ガイド | 中 |
| 7 | 契約書・見積書テンプレート | 低 |
...
| 講義形態 | 団体講義（10〜20名）前提に進行を再設計。カリキュラム骨格は据え置き |
| 進行設計原則 | 統合10原則（書籍×映画×教育）。正本は `design/integrated-engagement-framework.md`。源流の7原則は `design/group-lecture-engagement.md` |
| 進行パッケージ | 軽量／標準／重厚の3種。本番運営は標準パッケージを採用 |
| 自分の業務テーマ選定 | **第1回冒頭で全8回を通して使う「自分の業務テーマ」を1つ選定してもらう運用**。第4回以降のペアワーク・自業務実践で前提となる |
| 第4回チーム編成 | **業種別**（同じ業種の参加者を同じチームに集める）。お題は所属業種で固定 |
